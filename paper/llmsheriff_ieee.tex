...
We evaluate the rule-based analyzer and an LLM behavioral judge on a corpus of 100
synthetic labelled traces. The rule engine reaches 96\% accuracy and 0.978 macro-F1.
The LLM alone reaches 68\% accuracy, but when both analyzers agree ($n{=}64$), accuracy
is 100\%; disagreement cases include all four Stalled traces missed by rules. The work
is exploratory; it does not claim to solve intent monitoring, but demonstrates one
reproducible hybrid approach to behavioral inference above conventional observability.
\end{abstract}
...
After normalisation, twelve features are computed: runtime, average latency, LLM call
count, tool call count, retry count, failure count, longest streak of repeated identical
actions, a progress score (success rate penalised for non-waiting repetition, capped at
1.0), waiting-action count and ratio (poll/status-check vocabulary), and maximum/mean
inter-event idle gaps. Idle-gap features separate tight work loops (Stalled) from
...
feature patterns are unambiguous---multiple failures, polling waits, extreme repetition
with low progress, or clear recovery after errors.

\textbf{LLM-based analyzer.} Rules can still struggle on borderline traces: loops with
moderate progress scores, or abandonment without hard failures. The LLM behavioral
analyzer receives the full trace and can reason about action semantics, stage
transitions, and whether observed activity plausibly advances the goal. The trade-off is
latency, API dependence, and less predictable behaviour across prompts.
...
analyzer after encoding waiting (poll/status-check) features and separating tight work
loops (Stalled) from long-idle drift (Abandoned). Overall accuracy is 96\% with
macro-F1 of 0.978.
...
Table~\ref{tab:confusion} summarises the remaining errors. Four Stalled traces were
classified as Executing when the repetition streak did not sufficiently reduce the
progress score below the stall threshold. Earlier versions of the rule set classified
all Waiting traces as Executing because polling vocabulary was absent; adding
\texttt{waiting\_action\_count}/\texttt{waiting\_action\_ratio} features and idle-gap
metrics removed that systematic failure mode. Distinguishing Stalled (tight work loops)
from Abandoned (long idle gaps) similarly removed Abandoned$\rightarrow$Stalled
confusions.
...
These residual errors still motivate the LLM behavioral analyzer: they occur on
borderline traces where thresholds alone are brittle.

\subsection{Hybrid Dual-Analyzer Evaluation}

We ran the NVIDIA Nemotron LLM judge on the same 100 traces in parallel with the rule
engine (\texttt{backend/scripts/evaluate\_hybrid.py}). Of 100 calls, 93 returned a
parsed LLM prediction and 7 fell back to a local heuristic after API errors; overall LLM
accuracy is therefore 68\% (71\% on the 93 successful API responses).
...
Rule engine & 0.96 & 0.978 & Hand-tuned features/thresholds \\
LLM judge & 0.68 & 0.650 & 7/100 API fallbacks included \\
Agreement subset ($n{=}64$) & 1.00 & --- & Both analyzers agree \\ \bottomrule
\end{tabular}
\end{table}

Agreement occurred on 64/100 traces (64\%). When analyzers agreed, accuracy was
\textbf{100\%} relative to synthetic labels. When they disagreed ($n{=}36$), the rule
engine was correct on 88.9\% of cases and the LLM on 11.1\%. Disagreements concentrated
on Abandoned (20; LLM often predicted Stalled), Completed (12; LLM often predicted
Executing), and Stalled (4). Importantly, on all four Stalled traces that the rule
engine missed (predicted Executing), the LLM predicted Stalled correctly---evidence that
disagreement can surface complementary strengths rather than only noise.

Correctness breakdown: both correct 64; rule-only correct 32; LLM-only correct 4;
both wrong 0. These results support treating agreement as a confidence signal and
disagreement as a diagnostic cue for human review, even when the LLM alone is weaker
than the tuned rule baseline on this synthetic corpus.
...
\subsection{Human Labeling Protocol}

To assess whether the state vocabulary is usable by humans, we prepared a stratified
40-trace labeling pack (8 traces each for Completed, Recovering, Waiting, Stalled, and
Abandoned) with an annotation rubric and blank rater sheets
(\texttt{paper/human\_study/}). Gold generator labels are held out from annotators.
Agreement will be reported with Cohen's $\kappa$ (pairwise) and Fleiss' $\kappa$
(multi-rater) using \texttt{backend/scripts/compute\_kappa.py}. Results from this study
are pending completion of independent annotations.

% -----------------------------------------------------------------------
\section{Limitations}

\begin{itemize}
\item Rule thresholds are hand-tuned on the synthetic generators, not learned from
independent labelled data; high synthetic accuracy should not be read as production
generalisability.
\item The synthetic corpus approximates real agent behaviour but is not drawn from
production systems.
\item Remaining Stalled$\rightarrow$Executing errors show that thresholded progress
scores remain brittle on borderline loops.
\item The LLM judge underperformed the tuned rule baseline on this corpus (notably
collapsing Abandoned into Stalled) and had 7/100 API fallbacks; hybrid value here is
agreement confidence and complementary Stalled catches, not LLM dominance.
\item A human inter-rater study protocol is in place, but completed multi-rater
$\kappa$ results are not yet reported.
\end{itemize}
...
Evaluation on 100 synthetic traces shows strong rule-engine performance after encoding
waiting and idle-gap features (accuracy 96\%, macro-F1 0.978). Hybrid evaluation finds
64\% analyzer agreement with 100\% accuracy on the agreement subset, while disagreements
capture complementary Stalled detections by the LLM and systematic Abandoned/Completed
confusions. Future work includes completing the human inter-rater study, collecting
labelled real-world traces, calibrated threshold learning, and prompt/model ablations for
the LLM judge.
